...
export class UserPostServices {
    ...
    addUsersBySuperAdmin(user) {
        this.requestObj.putRequestObject(user);
        let userObjWithToken = this.requestObj.getRequestObject();

        return this.http.post('http://localhost:9191/users/addUser/admin', userObjWithToken, {
            responseType: 'json'
        });
    }
    ...
    //GET
    fetchUsers(userId) {
        return this.http.get(`http://localhost:9191/users/${userId}`,
            {
                responseType: 'json'
            }).
    ...
    getUserById(userDetailId: number) {
        return this.http.get<any>(`http://localhost:9191/users/userDetail/${userDetailId}`,
            {
                responseType: 'json'
            }).
            pipe(map((responseData: any) => {
    ...
    setUserInactive(userDetailId: number) {
        return this.http.delete(`http://localhost:9191/users/delete/${userDetailId}`);
    }
